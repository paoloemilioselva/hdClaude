// hdClaude genglsl_pt override of image_vector3.
//
...
void mx_image_vector3(HdclaudeTexture tex_sampler, int layer, vec3 defaultval, vec2 texcoord, int uaddressmode, int vaddressmode, int filtertype, int framerange, int frameoffset, int frameendaction, vec2 uv_scale, vec2 uv_offset, out vec3 result)
{
    vec2 uv = mx_transform_uv(texcoord, uv_scale, uv_offset);
    result = hdclaude_sample_image(tex_sampler, uv, vec4(defaultval, 1.0)).rgb;
}
