// hdClaude genglsl_pt override of lib/mx_microfacet_diffuse.glsl.
//
// MaterialX 1.39.3's file, verbatim, with one function removed:
// `mx_subsurface_scattering_approx`.
//
// That function estimates surface curvature from screen-space derivatives:
//
//     float curvature = length(fwidth(N)) / length(fwidth(P));
//
// Screen-space derivatives do not exist in a wavefront path tracer, and would
// be meaningless if they did: neighbouring lanes in a `shade` dispatch are
// unrelated paths that may be on opposite sides of the scene, not adjacent
// pixels. `fwidth` there measures nothing.
//
// It is removed rather than stubbed. Its only caller is `mx_subsurface_bsdf`,
// which hdClaude will override with a real bounded spectral random walk -- the
// transport this function approximates. Until then, any code path that reaches
// it fails to compile with the function named, which is the correct loud
// failure. Enabling GL_KHR_compute_shader_derivatives instead would make it
// compile and silently produce nonsense.
//
// Everything else in the file is upstream's and is used unchanged.
