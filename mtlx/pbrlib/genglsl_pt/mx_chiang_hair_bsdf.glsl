// hdClaude genglsl_pt override of chiang_hair_bsdf.
//
// MaterialX 1.39.3's file, verbatim, except for two changes at the end:
// CLOSURE_TYPE_INDIRECT is dropped, and PT_SAMPLE plus the density are added.
// Every helper below -- the Bessel approximations, the logistic azimuthal
// scattering, the attenuation terms -- is upstream's and untouched.
//
// Upstream's own comment on the branch removed here reads "This indirect term
// is a *very* rough approximation", which is a fair description of a rasteriser
// standing in for transport it cannot do, and exactly what a path tracer has no
// use for.
//
// SAMPLING IS UNBIASED BUT NOT YET MATCHED TO THE LOBES.
//
// A hair BCSDF scatters into the full sphere across four lobes (R, TT, TRT and
// the residual), and sampling it well means importance sampling the
// longitudinal and azimuthal distributions per lobe. This implementation uses a
// uniform-sphere proposal instead: the *evaluation* is exact, so the estimator
// is unbiased and the image converges to the right answer, but it converges
// more slowly than it should. That is a variance cost, not a correctness one,
// and it is confined to hair.
//
// Recorded as a known gap rather than hidden: no gallery scene uses hair, and
// matched lobe sampling is worth doing when one does. The furnace and
// chi-squared tests in docs/materialx-codegen.md 8 will pass on this
// implementation, because they test correctness; a variance measurement is what
// would show the gap.
                         ...
                         vec3 absorption_coefficient, vec3 N, vec3 X, inout BSDF bsdf)
{
        ...
        closureData.closureType == CLOSURE_TYPE_TRANSMISSION)
    {
        ...
        // Matches the uniform-sphere proposal above. When lobe-matched sampling
    ...
    // scatters forward and back, so a hemisphere proposal would miss the TT and
    // TRT lobes entirely, which would be a bias rather than merely noise.
    else if (closureData.closureType == CLOSURE_TYPE_PT_SAMPLE)
    {
        vec3 X2, Y2;
        mx_pt_basis(N, X2, Y2);
        vec3 local = mx_pt_sample_uniform_sphere(hdclaude_sample_u.xy);
        bsdf.sampledL = mx_pt_to_world(local, X2, Y2, N);
        bsdf.isDelta = 0.0;
    }
}
