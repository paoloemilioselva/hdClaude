...
| 6 | Spectral transport, lights, MIS, film | In progress | film accumulates progressively across render-pass executes. Remaining: hero-wavelength transport, UsdLux lights, MIS |
| 7 | Hydra delegate: adapters, render pass, AOVs, settings | In progress | plugin discovered as "Claude GPU Path Tracer"; mesh, material, camera, render-buffer adapters; progressive render pass; `gallery/shader_ball_gold.usda` renders through `usdrecord`. Remaining: UsdLux lights, textures, per-face subsets, face-varying UVs |
