...
| 6 | Spectral transport, lights, MIS, film | In progress | analytic UsdLux rect, disk, sphere and distant lights sampled by NEE; dome light supplies the environment; film accumulates progressively. Remaining: hero-wavelength transport, MIS, light-power selection, dome and IES textures |
| 7 | Hydra delegate: adapters, render pass, AOVs, settings | In progress | plugin discovered as "Claude GPU Path Tracer"; mesh, material, light, camera and render-buffer adapters; progressive render pass; all three `gallery/shader_ball_*.usda` scenes render lit and materially distinct; textures load through Hio and bind bindlessly. Remaining: per-face subsets, face-varying UVs, per-texture wrap and filter modes, depth and id AOVs |
