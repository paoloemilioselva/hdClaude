...
| 7 | Hydra delegate: adapters, render pass, AOVs, settings | In progress | plugin discovered as "Claude GPU Path Tracer"; mesh, material, light, camera and render-buffer adapters; progressive render pass; all three `gallery/shader_ball_*.usda` scenes render lit and materially distinct in ~6 s at 512x512, 128 spp. Remaining: textures, per-face subsets, face-varying UVs, depth and id AOVs |
